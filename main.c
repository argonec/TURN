...
int main(void)
{
  ...
  while (1)
  {
		...
		if(zad_on()) status_pov = HAZARD;
		 else prev=1, status_pov = TURN_OFF ; /// 
...
// }
		 ...
		 if((status_pov==TURN_PRAV) || (status_pov==BROKEN_PRAV)) ledr0, pov_lev0 ;
	 
   if(status_pov!=TURN_OFF&&prev==0)
	 {
		 __HAL_TIM_SET_COUNTER(&htim14, 0);
 		 prev=1;  
		 if(status_pov==TURN_LEV) ledr1, pov_lev1, ledg0, pov_prav0;
		 if(status_pov==TURN_PRAV) ledg1, pov_prav1,ledr0, pov_lev0 ;
	 }
...
////////////////////////  HAZARD   /////////////////////////////////////////////////
 ...
 /////////////////////// USART  /////////////////////////////////////////
		 if (uart_data_ready)
			 { 
            if (uart_data_received < -20) ledr1; 
					  else   ledr0;   
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, uderjanie_Pin|led1_Pin|led2_Pin|led3_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : uderjanie_Pin led1_Pin led2_Pin led3_Pin */
  GPIO_InitStruct.Pin = uderjanie_Pin|led1_Pin|led2_Pin|led3_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_PULLDOWN;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);
